...
void FilterDisplay::paint(juce::Graphics& g)
{
    ...
    float maxFreq = 20000.0f; // Максимальное значение частоты
    ...
    juce::Point<float> topPoint(highCutX, height / 2.7); // Изменяем положение верхней точки
    ...
    g.drawLine(lowCutPoint.getX(), lowCutPoint.getY(), topPoint.getX(), topPoint.getY(), 4.0f); // Рисуем линию до верхней точки
    g.drawLine(topPoint.getX(), topPoint.getY(), highCutPoint.getX(), highCutPoint.getY(), 4.0f); // Рисуем линию от верхней точки
    ...
    g.fillEllipse(topPoint.getX() - radius, topPoint.getY() - radius, 2 * radius, 2 * radius); // Рисуем круг сверху
